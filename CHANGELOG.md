...
- **상세 페이지 intro 섹션 확장**: contentTypeId별 전용 섹션 표시
  ...
  - 숙박(32): 체크인/체크아웃, 객실 수/유형, 부대시설, 편의시설 배지
  - 음식점(39): 대표 메뉴, 취급 메뉴, 포장 가능, 좌석 수
  - 쇼핑(38): 판매 품목, 매장 안내, 장서는 날
  - 레포츠(28): 예약 안내, 이용 요금, 개장 기간
  - 문화시설(14): 이용 요금, 할인 정보, 관람 소요시간
  - 공통: 문의처, 편의 체크(유모차/신용카드/반려동물), 문화유산
- **상세 페이지 info 렌더링 분기**: 여행코스(CourseSteps UI) / 숙박 객실(RoomCards UI) / 범용 자동 분기
- **숙박 객실 카드**: 객실 이미지, 인원, 비수기/성수기 요금, 시설 배지(TV, Internet, A/C 등)
- **반려동물 정보 4개 필드 추가**: 비치 품목, 구비 시설, 구매 품목, 렌탈 품목
- **번역 키 ~40개 추가**: ko/en 모두 contentTypeId별 라벨 번역

### Fixed
- **로케일 전환 시 search params 유실**: `LocaleSwitcher`에서 `useSearchParams` 추가하여 필터 상태(카테고리/지역 등) 보존 — 두 번 클릭해야 전환되던 버그 수정
- **문화유산 "0" 값 표시**: heritage 필드의 "0" 값을 필터링하여 의미없는 문화유산 섹션 숨김
